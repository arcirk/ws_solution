...
ws_drv::ws_drv()
{
    ...
    client = new IClient(settings[bConfFields::ServerHost].get_string(), settings[bConfFields::ServerPort].get_int(), callback, callback_status);
    _pwdEdit = false;
    ...
    port = std::make_shared<variant_t>((int)settings[bConfFields::ServerPort].get_int());
    AddProperty(L"port", L"Порт", port);
    server = std::make_shared<variant_t>(settings[bConfFields::ServerHost].get_string());
    ...
    AddProperty(L"user_uuid", L"ИдентификаторПользователя", [&](){
        return std::make_shared<variant_t>((client->get_user_uuid()));
    });

    AddProperty(L"client_uuid", L"ИдентификаторКлиента", [&](){
        return std::make_shared<variant_t>(client->get_app_uuid());
    });
