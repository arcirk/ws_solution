...
{
    QFile file(_setings[MplProfilesIniFile].toString());
    if(!file.exists())
        return;

    _profilesNames.clear();

    QSettings ini = QSettings(QDir::fromNativeSeparators(_setings[MplProfilesIniFile].toString()), QSettings::IniFormat);
