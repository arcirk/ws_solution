...
public class QShareUtils
{
    ...
    private static String AUTHORITY="ru.arcirk.java.sharex.fileprovider";

    protected QShareUtils()
    {
       //Log.d("arcirk", "QShareUtils()");
    }
    ...
    public static boolean checkMimeTypeView(String mimeType) {
        ...
        if (myIntent.resolveActivity(QtNative.activity().getPackageManager()) != null) {
            Log.d("arcirk checkMime ", "YEP - we can go on and View");
            return true;
        } else {
            Log.d("arcirk checkMime", "sorry - no App available to View");
        }
        return false;
    }
    ...
    public static boolean checkMimeTypeEdit(String mimeType) {
        ...
        // Verify that the intent will resolve to an activity
        if (myIntent.resolveActivity(QtNative.activity().getPackageManager()) != null) {
            Log.d("arcirk checkMime ", "YEP - we can go on and Edit");
            return true;
        } else {
            Log.d("arcirk checkMime", "sorry - no App available to Edit");
        }
        return false;
    }
    ...
    public static boolean share(String text, String url) {
        ...
        if (sendIntent.resolveActivity(QtNative.activity().getPackageManager()) != null) {
            ...
            Log.d("arcirk share", "Intent not resolved");
        }
        return false;
    }
    ...
    public static boolean createCustomChooserAndStartActivity(Intent theIntent, String title, int requestId, Uri uri) {
        ...
        if(defaultAppInfo == null) {
            Log.d("arcirk", title+" PackageManager cannot resolve Activity");
            return false;
        }
        ...
        // if (!defaultAppInfo.activityInfo.name.endsWith("ResolverActivity") && !defaultAppInfo.activityInfo.name.endsWith("EditActivity")) {
            // Log.d("arcirk", title+" defaultAppInfo not Resolver or EditActivity: "+defaultAppInfo.activityInfo.name);
            // return false;
        //}
        ...
        if (appInfoList.isEmpty()) {
            Log.d("arcirk", title+" appInfoList.isEmpty");
            return false;
        }
        Log.d("arcirk", title+" appInfoList: "+appInfoList.size());
        ...
        for (ResolveInfo appInfo : appInfoList) {
            ...
            if(isLowerOrEqualsKitKat) {
                Log.d("arcirk", "legacy support grantUriPermission");
        ...
        if (targetedIntents.isEmpty()) {
            Log.d("arcirk", title+" targetedIntents.isEmpty");
            return false;
        }
        ...
        if (targetedIntents.isEmpty()) {
            Log.d("arcirk", title+" only one Intent left for Chooser");
        } else {
        ...
        Log.d("arcirk", title+" Chooser Intent not resolved. Should never happen");
        return false;
    }
    ...
    public static boolean sendFile(String filePath, String title, String mimeType, int requestId) {
        ...
        try {
            ...
            Log.d("arcirk sendFile - cannot be shared: ", filePath);
            return false;
        }

        Log.d("arcirk sendFile", uri.toString());
        sendIntent.putExtra(Intent.EXTRA_STREAM, uri);

        if(mimeType == null || mimeType.isEmpty()) {
            // fallback if mimeType not set
            mimeType = QtNative.activity().getContentResolver().getType(uri);
            Log.d("arcirk sendFile guessed mimeType:", mimeType);
        }  else {
            Log.d("arcirk sendFile w mimeType:", mimeType);
        }
    ...
    public static boolean viewFile(String filePath, String title, String mimeType, int requestId) {
        ...
        try {
            ...
            Log.d("arcirk viewFile - cannot be shared: ", filePath);
            return false;
        }
        // now we got a content URI per ex
        // content://ru.arcirk.java.sharex.fileprovider/my_shared_files/qt-logo.png
        // from a fileUrl:
        // /data/user/0/ru.arcirk.java.sharex/files/share_example_x_files/qt-logo.png
        Log.d("arcirk viewFile from file path: ", filePath);
        Log.d("arcirk viewFile to content URI: ", uri.toString());

        if(mimeType == null || mimeType.isEmpty()) {
            // fallback if mimeType not set
            mimeType = QtNative.activity().getContentResolver().getType(uri);
            Log.d("arcirk viewFile guessed mimeType:", mimeType);
        } else {
            Log.d("arcirk viewFile w mimeType:", mimeType);
        }
    ...
    public static boolean editFile(String filePath, String title, String mimeType, int requestId) {
        ...
        try {
            ...
            Log.d("arcirk editFile - cannot be shared: ", filePath);
            return false;
        }
        Log.d("arcirk editFile", uri.toString());

        if(mimeType == null || mimeType.isEmpty()) {
            // fallback if mimeType not set
            mimeType = QtNative.activity().getContentResolver().getType(uri);
            Log.d("arcirk editFile guessed mimeType:", mimeType);
        } else {
            Log.d("arcirk editFile w mimeType:", mimeType);
        }
        ...
        try {
                ...
                if (iStream != null) {
                    ...
                    if (name != null) {
                        ...
                        Log.d("arcirk - create File", filePath);
                        File f = new File(filePath);
                        FileOutputStream tmp = new FileOutputStream(f);
                        Log.d("arcirk - create File", "new FileOutputStream");
