...
#include "ws_drv_synh.h"

std::string ws_drv_synh::extensionName() {
    return "WebSocketSynh";
}

ws_drv_synh::ws_drv_synh() {
    ...
    AddMethod(L"Add", L"Сложить", this, &ws_drv_synh::add);
    AddMethod(L"Message", L"Сообщить", this, &ws_drv_synh::message);
    AddMethod(L"CurrentDate", L"ТекущаяДата", this, &ws_drv_synh::currentDate);
    AddMethod(L"Assign", L"Присвоить", this, &ws_drv_synh::assign);
    AddMethod(L"SamplePropertyValue", L"ЗначениеСвойстваОбразца", this, &ws_drv_synh::samplePropertyValue);
    ...
    //        def_args.insert({0, 5});
    //        AddMethod(u"Sleep", u"Ожидать", this, &ws_drv_synh::sleep, std::move(def_args));
    //
    AddMethod(L"Sleep", L"Ожидать", this, &ws_drv_synh::sleep, {{0, 5}});
...
variant_t ws_drv_synh::add(const variant_t &a, const variant_t &b) {
...
void ws_drv_synh::message(const variant_t &msg) {
...
void ws_drv_synh::sleep(const variant_t &delay) {
    using namespace std;
